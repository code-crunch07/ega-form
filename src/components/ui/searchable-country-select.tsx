...
export function SearchableCountrySelect({
  ...
  return (
    <Popover.Root open={isOpen} onOpenChange={setIsOpen}>
      ...
      {/* Floating Dropdown Popover mounted via Portal to prevent container clipping */}
      <Popover.Portal>
        <Popover.Positioner
          side="bottom"
          align="start"
          sideOffset={6}
          className="isolate z-50 w-[var(--anchor-width)] min-w-[280px] max-w-[95vw] outline-none"
        >
          <Popover.Popup className="relative isolate z-50 max-h-72 w-full overflow-hidden rounded-2xl bg-white text-slate-900 shadow-2xl border border-slate-200/90 p-1.5 duration-100 font-jost outline-none animate-in fade-in-0 zoom-in-95">
            {/* Search Box Header */}
            <div className="p-2 border-b border-slate-100 bg-slate-50/70 rounded-xl mb-1">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full h-9 pl-9 pr-3 text-xs bg-white border border-slate-200 rounded-lg placeholder-slate-400 text-slate-800 focus:outline-hidden focus:border-[#252D65] focus:ring-1 focus:ring-[#252D65]"
                />
              </div>
            </div>
