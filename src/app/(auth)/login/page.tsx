...
export default function LoginPage() {
  ...
  const isFormValid = Boolean(watchEmail?.trim() && watchPassword);
  ...
  return (
    <Card className="w-full max-w-[420px] mx-auto shadow-2xl border-0 border-none ring-0 bg-white text-black rounded-lg overflow-hidden py-4 px-2 animate-entrance">
      <CardHeader className="space-y-4 flex flex-col items-center pb-2">
        ...
        {/* EGA Configurable Service Notice Slot (F-002) */}
        <div className="w-full p-3 bg-blue-50/80 border border-blue-200/80 rounded-xl text-left flex items-start gap-2.5 shadow-2xs">
          <span className="font-bold text-[10px] bg-[#27295B] text-white px-2 py-0.5 rounded-md uppercase font-mono tracking-wider shrink-0 mt-0.5">Notice</span>
          <p className="text-[11px] leading-relaxed text-slate-700 font-medium">
            Educare Global Academy (EGA) Admissions Portal. Intakes for academic year 2026/2027 are currently accepting applications.
          </p>
        </div>
      ...
      <CardContent className="pb-4">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          {/* Sign in button enabled only when required credentials present (F-001) */}
          <Button 
            type="submit" 
            className="w-full bg-[#3b5998] hover:bg-[#2d4373] text-white h-11 font-normal transition-all disabled:opacity-50 disabled:cursor-not-allowed" 
        ...
        <div className="grid grid-cols-2 gap-3">
          <Button 
            ...
            className="bg-white border-gray-300 text-black hover:text-black hover:bg-gray-50 h-11 flex items-center justify-center gap-2 cursor-pointer"
          >
