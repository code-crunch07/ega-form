...
import { MoreHorizontal, Eye, FileText, CheckCircle2, Clock, XCircle, RotateCcw, Trash2, Loader2 } from "lucide-react";
import { updatePaymentStatus, deletePayment } from "@/app/actions/admin";
...
export function PaymentActionsDropdown({ payment }: PaymentActionsDropdownProps) {
  const router = useRouter();
  const [isProcessing, setIsProcessing] = useState(false);

  const handleSetStatus = async (newStatus: "Paid" | "Pending" | "Failed" | "Refunded") => {
    setIsProcessing(true);
    const res = await updatePaymentStatus(payment.id, newStatus);
  ...
  const handleDelete = async () => {
    if (confirm(`Are you sure you want to delete payment record #${payment.invoiceNumber}? This action cannot be undone.`)) {
      setIsProcessing(true);
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-56 rounded-xl bg-white border-slate-200 shadow-xl p-1.5 font-jost text-left">
        <Link href={`/admin/payments/${payment.id}`} className="w-full">
        ...
        {payment.applicationId && (
          <Link href={`/admin/applications/${payment.applicationId}`} className="w-full">
            <DropdownMenuItem className="cursor-pointer py-2 px-2.5 rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-100 hover:text-[#252D65] focus:bg-slate-100 focus:text-[#252D65]">
